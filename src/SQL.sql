...
-- 3. Выведите группы их курсы
    select distinct group_id from Courses;

-- 4. Выведите курсы групп у которых курс начался с 202011 по 202333
    select g.*, c.*
        from courses c join public.groups g on g.id = c.group_id
...
-- 2. Вывести все уроки курса 'Technical English'
    select * from Courses join Lessons L on Courses.id = L.course_id
    where lesson_name = 'Technical English';

-- 3. Вывести количество всех студентов курса id  4
    select count(s.id)
    from Courses c
    join Students s on c.group_id = s.id
    where c.id > 4;
